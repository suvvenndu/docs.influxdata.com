...
InfluxDB's HTTP API and the [command line interface](/influxdb/v1.6/tools/shell/) (CLI), which connects to the database using the API, include simple, built-in authentication based on user credentials.
...
There are two options for authenticating with the [HTTP API](/influxdb/v1.6/tools/api/).
...
There are three options for authenticating with the [CLI](/influxdb/v1.6/tools/shell/).
...
Non-admin users can [`SHOW`](/influxdb/v1.6/query_language/schema_exploration/#show-databases) the databases on which they have `READ` and/or `WRITE` permissions.
...
* The password [string](/influxdb/v1.6/query_language/spec/#strings) must be wrapped in single quotes.
...
```
    ...
    > **Note:** The password [string](/influxdb/v1.6/query_language/spec/#strings) must be wrapped in single quotes.
Do not include the single quotes when authenticating requests.
> For passwords that include a single quote or a newline character, escape the single quote or newline character with a backslash both when creating the password and when submitting authentication requests.
