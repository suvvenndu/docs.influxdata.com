...
If you're looking for `SHOW` queries (for example, `SHOW DATABASES` or `SHOW RETENTION POLICIES`), see [Schema Exploration](/influxdb/v1.6/query_language/schema_exploration).

The examples in the sections below use InfluxDB's [Command Line Interface (CLI)](/influxdb/v1.6/introduction/getting-started/).
...
The `DROP SERIES` query deletes all points from a [series](/influxdb/v1.6/concepts/glossary/#series) in a database,
